const { formatNum, splitText, renderLevelIcon, renderTextLines } = require('../utils/common');
...
const generateSVG = (data, palette) => {
    ...
    return `
<svg width="813" height="709" viewBox="0 0 813 709" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" overflow="hidden">
    <defs>
        <style type="text/css">
            ${styleString}
        </style>
        ...
        <image width="1017" height="572" href="${data.video ? data.video.cover : ''}" xlink:href="${data.video ? data.video.cover : ''}" preserveAspectRatio="none" id="img2"></image>
        <clipPath id="clip3">
            <path
        ...
        <image width="233" height="233" href="${data.face}" xlink:href="${data.face}" preserveAspectRatio="none" id="img6"></image>
        <clipPath id="clip7">
            <path
    ...
    <g clip-path="url(#clip1)" transform="translate(-120 -97)">
        ...
        ${data.video && !data.video.error ? `
        <g clip-path="url(#clip3)">
            <use width="100%" height="100%" href="#img2" xlink:href="#img2" transform="matrix(0.758112 0 0 0.760489 141 191)"></use>
        </g>
        <rect x="141" y="338" width="771" height="291" fill="url(#fill4)" />
        ` : `
        <rect x="141" y="338" width="771" height="291" fill="${palette.cardBg}" opacity="0.3" />
        <text class="video-title" fill="${palette.textColor2}" opacity="0.5" transform="matrix(1 0 0 1 359.801 499)">视频数据加载失败</text>
        `}
        ...
        <g clip-path="url(#clip7)">
            <use width="100%" height="100%" href="#img6" xlink:href="#img6" transform="matrix(0.630902 0 0 0.635192 184 399)"></use>
        </g>
        <path
            ...
            fill-rule="evenodd" />
        <text class="title" transform="matrix(1 0 0 1 195.31 614)">${data.name}</text>
        <text class="label" transform="matrix(1 0 0 1 584.619 557)">关注</text>
        <text class="label" transform="matrix(1 0 0 1 584.619 622)">粉丝</text>
        <text class="label" transform="matrix(1 0 0 1 584.619 687)">获赞</text>
        <text class="stat" transform="matrix(1 0 0 1 678.235 555)">${formatNum(data.following)}</text>
        <text class="stat" transform="matrix(1 0 0 1 678.235 620)">${formatNum(data.follower)}</text>
        <text class="stat" transform="matrix(1 0 0 1 678.235 685)">${formatNum(data.like)}</text>
        ${renderTextLines(signatureLines, 'signature', 195.309, 659, 22)}
        <g transform="translate(305 535) scale(0.8)">
            ${renderLevelIcon(data.level, data.levelIcon, palette.accentColor)}
        </g>
        ${data.video && !data.video.error ? `
        <text class="video-title" font-family="Microsoft YaHei,Microsoft YaHei_MSFontService,sans-serif"
            transform="matrix(1 0 0 1 359.801 499)">${videoTitleLines[0]}</text>
        <text class="video-label" transform="matrix(1 0 0 1 359.801 474)">最新视频</text>
        ` : ''}
        <text class="footer" transform="matrix(1 0 0 1 456.901 785)">bili</text>
        <text class="footer" transform="matrix(1 0 0 1 483.829 785)">-</text>
        <text class="footer" transform="matrix(1 0 0 1 491.849 785)">card © 2026</text>
        <path
