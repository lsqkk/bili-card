...
const generateSVG = (data, palette) => {
    ...
    return `
<svg width="885" height="535" viewBox="0 0 885 535" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" overflow="hidden">
    <defs>
        <style type="text/css">
            ${styleString}
        </style>
        <image width="233" height="233" href="${data.face}" xlink:href="${data.face}" preserveAspectRatio="none" id="img0"></image>
        <clipPath id="clip1">
            <path
        ...
        <image width="509" height="286" href="${data.video ? data.video.cover : ''}" xlink:href="${data.video ? data.video.cover : ''}" preserveAspectRatio="none" id="img2"></image>
        <clipPath id="clip3">
            <path
    ...
    <g transform="translate(1 1)">
        ...
        <g clip-path="url(#clip1)">
            <use width="100%" height="100%" href="#img0" xlink:href="#img0" transform="matrix(0.635192 0 0 0.6309 38.0002 343)">
            </use>
        </g>
        <path
        ...
        <text class="label" transform="matrix(1 0 0 1 55 76)">关注</text>
        <text class="label" transform="matrix(1 0 0 1 55 174)">粉丝</text>
        <text class="label" transform="matrix(1 0 0 1 55 276)">获赞</text>
        <text class="stat" transform="matrix(1 0 0 1 151 78)">${formatNum(data.following)}</text>
        <text class="stat" transform="matrix(1 0 0 1 151 175)">${formatNum(data.follower)}</text>
        <text class="stat" transform="matrix(1 0 0 1 151 278)">${formatNum(data.like)}</text>
        ${renderTextLines(signatureLines, 'signature', 223.781, 449, 28)}
        <g transform="translate(145 462)">
            ${renderLevelIcon(data.level, data.levelIcon, palette.accentColor)}
        </g>
        ${data.video && !data.video.error ? `
        <g clip-path="url(#clip3)">
            <use width="100%" height="100%" href="#img2" xlink:href="#img2" transform="matrix(0.711198 0 0 0.709791 388 96.9999)">
            </use>
        </g>
        <path
            ...
            stroke="${palette.strokeColor}" stroke-width="2.29167" stroke-linejoin="round" stroke-miterlimit="10" fill="none"
            fill-rule="evenodd" />
        ${renderTextLines(videoTitleLines, 'video-title', 390.744, 56, 22)}
        ` : data.video?.error ? `<text class="video-title" fill="${palette.textColor2}" opacity="0.5" transform="matrix(1 0 0 1 390.744 56)">视频数据加载失败</text>` : ''}
        ${generateFooter(623.165, 513, 'footer', palette.accentColor, 18)}
